...
namespace Hangfire.PostgreSql
{
    ...
    {
        private readonly Queue<Action<PgSqlConnection>> _commandQueue = new Queue<Action<PgSqlConnection>>();
...
";
            ...
            QueueCommand((con) => con.Execute(
                sql
                .Replace("@id", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")));
        }
...
";
            ...
            QueueCommand((con) => con.Execute(
                ...
                .Replace("@createdAt", $@"'{DateTime.UtcNow}'")
                .Replace("@data", $@"'{SerializationHelper.Serialize(state.SerializeData())}'")
                .Replace("@id", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")));
        }
...
";
            ...
            QueueCommand((con) => con.Execute(
                trimSql
                .Replace("@key", $@"'{key}'")
                .Replace("@start", $@"'{keepStartingFrom}'")
                .Replace("@end", $@"'{keepEndingAt - keepStartingFrom + 1}'")));
        }
...
";
        ...
        public override void ExpireSet(string key, TimeSpan expireIn)
        {
            ...
            QueueCommand((connection) => connection.Execute(
                sql
                .Replace("@key", $@"'{key}'")
                .Replace("@expireAt", $@"'{DateTime.UtcNow.Add(expireIn)}'")));
        }
        ...
        internal void QueueCommand(Action<PgSqlConnection> action)
        {
