...
WHERE ""id"" = @id;";
                ...
                var sqlParameters = @"SELECT ""jobid"" ""JobId"", ""name"" ""Name"", ""value"" ""Value"" from """ + _options.SchemaName + @""".""jobparameter"" 
WHERE ""jobid"" = @id; ";

                var sqlHistory = @"SELECT ""jobid"" ""JobId"", ""name"" ""Name"", ""reason"" ""Reason"", ""createdat"" ""CreatedAt"", ""data"" ""Data"" 
...
ORDER BY ""id"" DESC; ";
                ...
                using (var multi = connection.QueryMultiple(sqlJob.Replace("@id", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")))
                {
                    job = multi.Read<SqlJob>().SingleOrDefault();
                    if (job == null) return null;
                }

                using (var multi = connection.QueryMultiple(sqlParameters.Replace("@id", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")))
                {
                    parameters = multi.Read<JobParameter>().ToDictionary(x => x.Name, x => x.Value);
                }

                using (var multi = connection.QueryMultiple(sqlHistory.Replace("@id", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")))
                {
                    history =
                        multi.Read<SqlState>()
                            .ToList()
...
GROUP BY ""statename"";";
                ...
                var serversSql = $@"SELECT COUNT(*) 
FROM ""{_options.SchemaName}"".""server"";";

                var succeededSql = $@"SELECT SUM(""value"") 
FROM ""{_options.SchemaName}"".""counter"" 
WHERE ""key"" = 'stats:succeeded';";

                var deletedSql = $@"SELECT SUM(""value"") 
FROM ""{_options.SchemaName}"".""counter"" 
WHERE ""key"" = 'stats:deleted';";

                var recurringJobsSql = $@"SELECT COUNT(*) 
FROM ""{_options.SchemaName}"".""set"" 
WHERE ""key"" = 'recurring-jobs';";

                var stats = new StatisticsDto();
                using (var multi = connection.QueryMultiple(statsSql))
                {
                ...
                using (var multi = connection.QueryMultiple(serversSql))
                {
                    stats.Servers = multi.Read<long>().Single();
                }

                using (var multi = connection.QueryMultiple(succeededSql))
                {
                    stats.Succeeded = multi.Read<long?>().SingleOrDefault() ?? 0;
                }

                using (var multi = connection.QueryMultiple(deletedSql))
                {
                    stats.Deleted = multi.Read<long?>().SingleOrDefault() ?? 0;
                }

                using (var multi = connection.QueryMultiple(recurringJobsSql))
                {
...
";
            ...
            var valuesMap = _storage.UseConnection(connection => connection.Query(
                query.Replace("@keys", "'{" + String.Join(",", keyMaps.Keys.ToList()) + "}'"))
                .ToList()
...
";
            ...
            var jobs = _storage.UseConnection(connection => connection.Query<SqlJob>(
                enqueuedJobsSql.Replace("@jobIds", "'{" + String.Join(",", jobIds.ToList()) + "}'"))
                .ToList());
...
";
            ...
            var jobs = _storage.UseConnection(connection => connection.Query<SqlJob>(
                fetchedJobsSql.Replace("@jobIds", "'{" + String.Join(",", jobIds.ToList()) + "}'"))
                .ToList());
