...
namespace Hangfire.PostgreSql
{
    internal class ExpirationManager : IBackgroundProcess, IServerComponent
    {
        ...
        public void Execute(CancellationToken cancellationToken)
        {
            ...
            {
                Logger.DebugFormat($"Removing outdated records from table '{table}'...");
...
WHERE ""id"" IN (
    ...
    LIMIT {1}
                    ...
                    if (removedCount > 0)
                    {
                        Logger.InfoFormat($"Removed {removedCount} outdated record(s) from '{table}' table.");
...
";
                    ...
                    if (aggregatedValue > 0)
                    {
                        ...
                        connection.Connection.Execute(insertQuery
                            .Replace("@key", $@"'{counterName}'")
                            .Replace("@value", $@"'{aggregatedValue}'"));
                    }
                }
            }
        }
