...
namespace Hangfire.PostgreSql
{
	public class PostgreSqlConnection : JobStorageConnection
	{
		...
		public PgSqlConnection Connection => _connection;
...
";
			...
			var jobId = _connection.Query<long>(
				createJobSql
				.Replace("@invocationData", $@"'{SerializationHelper.Serialize(invocationData)}'")
				.Replace("@arguments", $@"'{invocationData.Arguments}'")
				.Replace("@createdAt", $@"'{createdAt}'")
				.Replace("@expireAt", $@"'{createdAt.Add(expireIn)}'"))
...
";
					...
					_connection.Execute(insertParameterSql
						.Replace("@jobId", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")
						.Replace("@name", $@"'{parameter.Key}'")
						.Replace("@value", $@"'{parameter.Value}'"));
				}
...
";
			...
			var jobData = _connection.Query<SqlJob>(sql
				.Replace("@id", $@"{Convert.ToInt32(id, CultureInfo.InvariantCulture)}"))
				.SingleOrDefault();
...
";
			...
			var sqlState = _connection.Query<SqlState>(sql
				.Replace("@jobId", $@"{Convert.ToInt32(jobId, CultureInfo.InvariantCulture)}")).SingleOrDefault();
			if (sqlState == null)
			{
				return null;
			}
...
WHERE NOT EXISTS (
	...
	AND ""updatedrows"".""name"" = ""insertvalues"".""name""
			...
			_connection.Execute(sql
				.Replace("@jobId", $@"{Convert.ToInt32(id, CultureInfo.InvariantCulture)}")
				.Replace("@name", $@"'{name}'")
				.Replace("@value", $@"'{value}'"));
		}
...
";
		...
		public override HashSet<string> GetAllItemsFromSet(string key)
		{
			...
			var result = _connection.Query<string>(query.Replace("@key", $@"'{key}'"));
...
";
			...
			while (execute)
			{
				try
				{
					using (var transaction = _connection.BeginTransaction(IsolationLevel.Serializable))
					{
						foreach (var keyValuePair in keyValuePairs)
						{
							_connection.Execute(sql
								.Replace("@key", $@"'{key}'")
								.Replace("@field", $@"'{keyValuePair.Key}'")
								.Replace("@value", $@"'{keyValuePair.Value}'"), transaction: transaction);
						}
						transaction.Commit();
						execute = false;
					}
				...
				{
					if (!exception.ErrorSql.Equals("40001"))
						throw;
				}
...
";
			...
			_connection.Execute(sql.Replace("@id", $@"'{serverId}'").Replace("@data", $@"'{SerializationHelper.Serialize(data)}'"));
		}
		...
		public override void Heartbeat(string serverId)
		{
			...
			int affectedRows = _connection.Execute(query.Replace("@id", $@"'{serverId}'"));
		...
		public override long GetSetCount(string key)
		{
			...
			return _connection.Query<long>(query.Replace("@key", $@"'{key}'")).First();
		}
		...
		public override List<string> GetAllItemsFromList(string key)
		{
			...
			return _connection.Query<string>(query.Replace("@key", $@"'{key}'")).ToList();
		}
		...
		public override long GetCounter(string key)
		{
			...
			return _connection.Query<long?>(query.Replace("@key", $@"'{key}'")).SingleOrDefault() ?? 0;
		}
		...
		public override long GetListCount(string key)
		{
			...
			return _connection.Query<long>(query.Replace("@key", $@"'{key}'")).SingleOrDefault();
		}
		...
		public override TimeSpan GetListTtl(string key)
		{
			...
			var result = _connection.Query<DateTime?>(query.Replace("@key", $@"'{key}'")).Single();
		...
		public override List<string> GetRangeFromList(string key, int startingFrom, int endingAt)
		{
			...
			return _connection.Query<string>(query
				.Replace("@key", $@"'{key}'")
				.Replace("@startingFrom", $@"{startingFrom + 1}")
				.Replace("@endingAt", $@"{endingAt + 1}"))
				.ToList();
		}
		...
		public override List<string> GetRangeFromSet(string key, int startingFrom, int endingAt)
		{
			...
			return _connection.Query<string>(query
				.Replace("@key", $@"'{key}'")
				.Replace("@startingFrom", $@"'{startingFrom + 1}'")
				.Replace("@endingAt", $@"'{endingAt + 1}'")).ToList();
		}
		...
		public override TimeSpan GetSetTtl(string key)
		{
			...
			var result = _connection.Query<DateTime?>(query.Replace("@key", $@"'{key}'")).SingleOrDefault();
			if (!result.HasValue) return TimeSpan.FromSeconds(-1);
		...
		public override string GetValueFromHash(string key, string name)
		{
			...
			return _connection.Query<string>(query
				.Replace("@key", $@"'{key}'")
				.Replace("@field", $@"'{name}'")).SingleOrDefault();
		}
	}
}
